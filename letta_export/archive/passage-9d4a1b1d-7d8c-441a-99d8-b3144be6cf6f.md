# SESSION CHUNK 2026-04-25 — Designing the Direct Memory Write Mechanism and the L

*ID: passage-9d4a1b1d-7d8c-441a-99d8-b3144be6cf6f*
*Created: 2026-05-17*

---

SESSION CHUNK 2026-04-25 — Designing the Direct Memory Write Mechanism and the Live Block Filing Feature
...
Investigation reveals that Letta already has live memory-writing capabilities: built-in memory tools (`core_memory_append`, `core_memory_replace`, `memory_rethink`, etc.) that agents can call during a session to update blocks in real-time. These tools automatically route through GitEnabledBlockManager when enabled, so writes commit to git immediately.

**Current Capability**: An external caller can send a message via `POST /v1/agents/{id}/messages` instructing the agent to update memory. The LLM (Haiku) decides which tool to call, and the write happens immediately. The git commit is created as part of the tool execution.

**Problem with the Current Path**: This is non-deterministic. The LLM has to parse the instruction, decide which tool to use, choose the right block label, and format the content. That's a chain of failure points and latency cost for something that should be deterministic.

**The Design**: MemShepherd exposes a structured memory-write endpoint (`POST /memory/write`) that accepts:
```json
{
  "block_label": "world/patterns",
  "operation": "append",  // or replace, rethink
  "content": "...",
  "direct": true  // skip LLM, write immediately
}
```

With `direct: true`, MemShepherd resolves the block label to a block ID and calls `PATCH /v1/blocks/{id}` directly. No Letta agent involved, no LLM reasoning, no prompt engineering. Fast, deterministic, structured. When `direct: false`, it routes through `POST /v1/agents/{id}/messages` for sleep-time reflection (where LLM reasoning is valuable).

**Architectural Boundary**: This design keeps the architectural principle intact: Claude Code (foreground) does not directly manage Letta memory. Instead, it sends a request to MemShepherd, which handles the actual memory operation. The separation of concerns is preserved, the distraction is minimized, but Daimon has the ability to flag something important for immediate recording without waiting for the next sleep-time consolidation cycle.

**Implementation Location**: MemShepherd is external to Letta (not in the fork), calling vanilla Letta APIs. When built, it will be a simple wrapper with no Letta fork dependency.
