# SESSION CHUNK 2026-06-26 — Badge UI Redesign and Placement

*ID: passage-5eb6e495-e496-4b8e-a4b5-abea13e4f856*
*Created: 2026-07-01*
...
SUMMARY
Amos objected to the agency approval badge styling and placement: they were inline with therapy type chips (Pharmacological) and displayed as small tags with "Approved" suffix. Redesigned to: (1) Move badges from hero row to their own dedicated row between the Share button and Mode of Action section; (2) Change styling from small inline tags to large primary-color pills with bold white text; (3) Remove the redundant "Approved" suffix — badges now display just the agency names ("FDA Approved", "EMA Approved", "ANVISA Approved"). Discovered a secondary issue: the `evidenceBadge` function was rendering "approved" as a separate chip next to Pharmacological, which was the "stray badge" Amos noticed. Fixed by suppressing the "approved" label from that function. After first iteration, Amos noted only FDA pill was showing even after running EMA monitor locally. This triggered deeper diagnostic work into the matching logic.
