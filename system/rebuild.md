...
- **Daimon** — the Letta agent persona, restored from Neon (already there, not rebuilt)
...
|---|---|---|
| Docker Desktop | https://www.docker.com/products/docker-desktop | Enable WSL2 backend when prompted |
| Git + Git Bash | https://git-scm.com/download/win | Git Bash is used for docker commands (avoid path translation issues with `MSYS_NO_PATHCONV=1`) |
| Python 3.11+ | https://www.python.org/downloads/ | Add to PATH during install |
| Claude Code CLI | https://claude.ai/code | Follow install instructions; requires Anthropic account |

### 1.2 Claude Code Authentication

Claude Code must authenticate via subscription (not API key billing):
...
All secrets are stored as **Windows User environment variables** (never in files). Set them via System Properties → Environment Variables → User variables, or via Python:
...
|---|---|---|
| `MemShepherd_Anthropic_API_KEY` | Anthropic API key (sk-ant-...) | Letta agent's LLM (Claude Haiku) — separate from subscription |
| `MemShepherd_Voyage_API_KEY` | Voyage AI key | Embeddings (voyage-3-lite, 512 dimensions) |
| `MemShepherd_Letta_PG_URI` | `postgresql://neondb_owner:<password>@ep-delicate-smoke-aps537eh.c-7.us-east-1.aws.neon.tech/neondb` | Neon cloud DB — get current password from Neon dashboard |

**Getting the Neon password:** Log into [neon.tech](https://neon.tech), open project `ep-delicate-smoke-aps537eh`, go to Connection Details, copy the connection string for user `neondb_owner`. The URI must NOT include `?sslmode=require` — Letta handles SSL internally.

**Note:** A new Anthropic API key for MemShepherd may need to be created at [console.anthropic.com](https://console.anthropic.com). This is separate from the subscription used for Claude Code. Letta uses it internally to run Haiku for memory consolidation.
...
The `memfs` volume is Letta's git-backed block storage. It must exist before the container starts.

---

## Part 6: Run the Container

Read the three env var values from the registry and pass them in (Git Bash):
...
- `update_world.py` — utility to patch world/patterns block (hardcoded content — update before use)
- `update_persona.py` — utility to patch persona block

### 7.3 Create session_sync.py (personal hook, not in public repo)

This hook exports Letta memory to anamnesis and pushes to GitHub after each session. It contains hardcoded IDs specific to the Daimon agent in Neon — these IDs are stable as long as the Neon database exists.

Create `~/.claude/memshepherd/hooks/session_sync.py` with content from `anamnesis/letta_export/blocks/` as reference for IDs, and the script structure described in MODIFICATIONS.md section 3. Key constants:
...
```python
BLOCK_FILES = {
    "system/persona": "persona.md",   # block-9e455fad-c9ec-436e-93f3-03223caa9290
    "system/human": "human.md",       # block-da58ddb2-4a6f-4103-b3f3-af2260b6f3d2
    "world/patterns": "world_patterns.md",  # block-69939755-6d23-41d2-a7bc-c5dd85067011
}
```

### 7.4 Apply Claude Code settings

Copy `config/claude_settings.json` from the MemShepherd repo to `~/.claude/settings.json`. This configures:
- Hook commands (SessionStart, PostToolUse, SessionEnd)
- Permission allowlist for hooks
- Browser automation (MCP) permissions
- `advisorModel: "opus"`

### 7.5 Apply CLAUDE.md

Copy `config/CLAUDE.md` from the MemShepherd repo to `~/.claude/CLAUDE.md`. This sets global behavioral rules for all Claude Code sessions.
...
This directory is machine-specific (keyed to the working directory). On a fresh machine, it will be empty — Claude Code will rebuild it over sessions. No manual restoration needed.
...
```bash
python3 "C:/Users/<username>/.claude/memshepherd/hooks/session_start.py"
```
...
```bash
MSYS_NO_PATHCONV=1 docker exec memshepherd-letta psql \
  "postgresql://neondb_owner:<password>@ep-delicate-smoke-aps537eh.c-7.us-east-1.aws.neon.tech/neondb" \
...
MCP server configs live in `~/.claude/settings.json` under the `mcpServers` key (separate from the MemShepherd hooks config). Restore from the previous machine's settings or reconfigure per each MCP server's documentation.

---

## Key IDs Reference (Stable — stored in Neon)

These IDs are in the Neon database and do not change unless the database is wiped and rebuilt.
...
**session_start returns empty or fails:** Letta may still be starting. Wait 15-30 seconds after container start. The hook has retry logic but may need more time.

**Vector search fails with "operator does not exist":** Run `ALTER ROLE neondb_owner SET search_path TO letta, public;` on Neon. This was done on 2026-05-08 and should persist — if it reverts, Neon may have reset the role config.

**Docker path issues in Git Bash:** Prefix any `docker exec` command with `MSYS_NO_PATHCONV=1` to prevent Git Bash from translating Linux paths to Windows paths.
